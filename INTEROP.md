...
How **prefetchparse** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `prefetchparse` lives in the **Host / artifact forensics** cluster.
...
| from | relation | to |
|---|---|---|
| `prefetchparse` | JSON findings compose with | [`memtriage`](https://github.com/cognis-digital/memtriage), [`evtxhunt`](https://github.com/cognis-digital/evtxhunt), [`browserforensics`](https://github.com/cognis-digital/browserforensics) |
| `prefetchparse` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `prefetchparse` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `prefetchparse` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the host / artifact forensics cluster.** `prefetchparse` output feeds the next tool:
```bash
prefetchparse ... --format json > out.json          # this tool's findings (see `prefetchparse --help`)
...
prefetchparse ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
prefetchparse ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
prefetchparse ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `prefetchparse`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `prefetchparse` slots
