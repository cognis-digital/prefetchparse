...
- **rules/analyzers** apply the heuristics shipped in `prefetchparse/core.py`.
- **score** ranks by severity.
- **MCP server** (`prefetchparse mcp`) exposes `scan` for Cognis.Studio agents.
